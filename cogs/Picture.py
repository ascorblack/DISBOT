...
class Picture(commands.Cog):
    ...
    #         soup = BeautifulSoup(rs.content, 'lxml')
    #         a = []
    #         for img in soup.find_all('a', src=False, href=True, id=False, class_=False, limit=60):
    #             check = img['href']
    #             if check[0] != '#' and img['href'] != '/counts/posts' and img['href'] != '/static/contact' and img['href'] != '/terms_of_service':
    #                 a.append(img['href'])
    #         num = len(a)
    ...
    #             out = []
    #             for imag in soup.find_all('img', alt=True, class_="fit-width", src=True):
    #                 out.append(imag['src'])
    ...
    #             soup = BeautifulSoup(root.content, 'lxml')
    #             out = []
    #             for imag in soup.find_all('video', alt=True, class_="fit-width", src=True):
    #                 out.append(imag['src'])
    #             await ctx.send(out[0])
    #     else:
    #         emb = discord.Embed(title=f'{ctx.author}, пожалуйста, напиши тэг')
    ...
    async def r34_error(self, ctx, error):
        if isinstance(error, commands.NSFWChannelRequired):
            ...
            await ctx.send(embed=emb)
    # @dan.error
    # async def dan_error(self, ctx, error):
    #     if isinstance(error, commands.NSFWChannelRequired):
    #         emb = discord.Embed(description=f':no_entry_sign: Канал **{ctx.channel}** не является NSFW!', color = discord.Colour.red())
    #         await ctx.send(embed=emb)
