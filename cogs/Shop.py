...
class Shop(commands.Cog):
    ...
    #         emb = discord.Embed(title="Магазин Ролей", color = discord.Colour.dark_magenta())
    ...
    #         emb = discord.Embed(title="Магазин Предметов", color = discord.Colour.dark_magenta())
    ...
    async def trans_error(self, ctx, error):
        if isinstance(error, commands.MissingRequiredArgument):
            synt = 'give/trans <@member> <item-name> <quantity>'
            await get_error(ctx, synt)
    @pay.error
    async def apy_error(self, ctx, error):
        if isinstance(error, commands.MissingRequiredArgument):
            synt = 'pay <@member> <quantity money>'
            await get_error(ctx, synt)
